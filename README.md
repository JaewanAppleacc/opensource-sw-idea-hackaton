...
- (`/manual-analysis` 합성 fixture 경로) 같은 직종·고용형태의 전북 공고 최대 3건 탐색
- (`/ai-job-recommend` 실제 데이터 경로) 수집 시 기록된 직종·고용형태가 동일한 실제 수도권·전북 공고를, 수집 순서에 따라 결정론적으로 1:1 연결한 비교 공고 1건 제공 — 유사도 검색이나 사람이 고른 "최적 후보"가 아니며, 현재 배치에서는 항상 최대 1건만 반환됨
...
기본 시연 동선은 **AI추천 목록 → 수도권 공고 선택 → 전북 비교 공고 연결 → 6개 항목 비교 → 확인 질문 → 선택적 자금 축적 비교**입니다. 수동 붙여넣기는 더 이상 기본 화면이 아니며 보조 경로(`/manual-analysis`)로만 남아 있습니다.

1. `/ai-job-recommend`에서 **데모 모드 · 사전 검증된 분석 결과** 배지, "본 서비스는 고용24 공식 서비스가 아닌 해커톤 시연용 프로토타입입니다." 문구, 그리고 로그인 전 안내("로그인하면 관심 생활권의 비교 공고를 확인할 수 있습니다.") 확인
...
3. 실제 수도권 채용공고 목록(회사명, 근무지역, 고용형태, 출처·원문 링크, 수집 기준일)이 표시됨 → 원하는 공고 카드에서 **내 지역 비교 공고 보기 (전북특별자치도 기준)** 클릭
4. 카드 안에서 바로 **비교 대상으로 연결된 기준**(수집 시 기록된 모집직종·고용형태가 같고, 수집 순서에 따라 결정론적으로 1:1 연결됐다는 사실, "가장 유사하거나 유일한 지역 대안이라는 의미는 아님"이라는 안내 포함)과 함께 전북 비교 공고 1건이 펼쳐짐 (유사도 점수·추천 순위 없음) → 후보 선택
...
- 프런트엔드는 고용24 공식 서비스가 아닌 비공식 학습용 클론입니다. `/ai-job-recommend`의 전북 비교 공고는 실제 20건 표본(`data/intake/real_postings.jsonl`)에서 가져오고, `/manual-analysis`의 전북 비교 후보는 합성 데모 데이터셋(`data/postings/postings.jsonl`)에서만 가져옵니다 — 두 경로 모두 실제 비공개 원문(`data/private/**`)은 절대 프런트에 그대로 노출하지 않으며, 서버 측에서만 조회해 분석 결과만 반환합니다.
