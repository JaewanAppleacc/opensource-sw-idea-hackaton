...
/**
 * Advanced/secondary path (TASK section 8): paste any metropolitan posting
 * body manually instead of picking one from the AI추천 목록. This used to
 * be the default landing screen at /ai-job-recommend; it is preserved here
 * (not deleted) because manual comparison of an arbitrary posting is still
 * a useful capability, just no longer the primary demo flow.
 */
    ...
    <div className="py-10 sm:py-14">
      <Container className="max-w-3xl">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
          ...
          <Link to="/" className="text-sm font-medium text-ink-400 transition hover:text-brand-blue">
            고용24 홈으로
          </Link>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-brand-blue">AI추천(일자리) · 수동 입력(고급)</p>
            <h1 className="text-xl font-bold text-ink-900 sm:text-2xl">채용공고 직접 붙여넣어 비교하기 (데모)</h1>
          </div>
        </div>
        ...
        <p className="mt-4 text-sm leading-relaxed text-ink-500">
          AI추천 목록에 없는 다른 수도권 채용공고를 직접 붙여넣으면 같은 직종·고용형태의 전북 비교 후보를 찾고, 두
          공고를 6개 항목으로 감사한 뒤 모호하거나 없는 조건을 확인 질문으로 보여드려요.
        </p>
            ...
            {selectedCandidate && (
              <section aria-labelledby="analysis-heading" className="space-y-3">
                ...
                <FieldComparisonTable
                  metroLabel="수도권 공고"
                  jeonbukLabel={`전북 후보 (${selectedCandidate.company_name ?? selectedCandidate.posting_id})`}
                  metro={metroAnalysis}
